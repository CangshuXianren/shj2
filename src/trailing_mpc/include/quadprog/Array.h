...
typedef enum MType { DIAG=0 }MType_;
typedef enum Mmultiply { SCALAR=0, CROSS=1 }Mmultiply_;
...
public:
  ...
  Matrix(MType_ t, const T& a, const T& o, const unsigned int n, const unsigned int m);
  Matrix(MType_ t, const Vector<T>& v, const T& o, const unsigned int n, const unsigned int m);
  ...
  inline void resize(MType_ t, const T& a, const T& o, const unsigned int n, const unsigned int m);
  ...
  inline Matrix<T> transpose(void);
  inline Matrix<T> getblock(const unsigned int row, const unsigned int colum, const unsigned int Nrow, const unsigned int Ncolum);
  inline bool setblock(const unsigned int row, const unsigned int colum, const Matrix<T>& w);
...
template <class T> 
Matrix<T>::Matrix(MType_ t, const T& a, const T& o, unsigned int n, unsigned int m)
  : v(new T*[n])
{ 
  this->n = n; this->m = m;
  v[0] = new T[m * n]; 
...
template <class T> 
Matrix<T>::Matrix(MType_ t, const Vector<T>& a, const T& o, unsigned int n, unsigned int m)
  : v(new T*[n])
{ 
  this->n = n; this->m = m;
  v[0] = new T[m * n]; 
...
template <typename T>
inline void Matrix<T>::resize(MType_ t, const T& a, const T& o, const unsigned int n, const unsigned int m)
{
	resize(n, m);
  	...
  	switch (t)
	{
		case DIAG:
			...
			break;
		default:
			std::cout<<"Matrix type not supported"<<std::endl;
			break;
	}
}
...
{
	const unsigned int new_n=ncols();//3
	const unsigned int new_m=nrows();//2
	Matrix<T> tmp(new_n, new_m);
...
template <typename T>
inline Matrix<T> Matrix<T>::getblock(const unsigned int row, const unsigned int colum, const unsigned int Nrow, const unsigned int Ncolum)
{
	Matrix<T> tmp(Nrow, Ncolum);
	if( (row+Nrow)>nrows() || (colum+Ncolum)>ncols() )
	{
		std::cout<<"Error in getblock: out of row or colum!"<<std::endl;
		return tmp;
	}
...
template <typename T>
inline bool Matrix<T>::setblock(const unsigned int row, const unsigned int colum, const Matrix<T>& w)
{
	unsigned int set_Nrow = w.nrows();
	unsigned int set_Ncolum = w.ncols();
	if( (row+set_Nrow)>nrows() || (colum+set_Ncolum)>ncols() )
	{
		std::cout<<"Error in setblock: out of row or colum!"<<std::endl;
		return false;
	}
	for(int j=0; j<set_Nrow; j++)
	{
		for(int k=0; k<set_Ncolum; k++)
		{
			v[row+j][colum+k] = w[j][k];
		}
	}
	return true;
}

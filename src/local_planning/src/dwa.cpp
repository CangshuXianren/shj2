...
double dt;                //运动学模型预测时间
double forecast_time; //prediction
double predict_time;
...
inline bool CheckDWAArrive(CarState now, CarState end){
    if(sqrt(pow(now.x - end.x, 2) + pow(now.y - end.y, 2)) <= overall_length) return true;
    else return false;
}
...
void DWA::findMatchPoint(const CarState &now_pos, const nav_msgs::Path refPath){
    //prediction
    CarState prediction_pos;
    prediction_pos.x = now_pos.x + now_pos.speed * forecast_time * cos(now_pos.yaw);
    prediction_pos.y = now_pos.y + (now_pos.speed) * forecast_time * sin(now_pos.yaw);    

    double mindis = 1 << 20;
    for(int i = iteration; i < refPath.poses.size(); ++i){
        double tmpdis = pow(prediction_pos.x - refPath.poses[i].pose.position.x, 2) + pow(prediction_pos.y - refPath.poses[i].pose.position.y, 2);
        if(tmpdis < mindis){
            mindis = tmpdis;
            iteration = i;
        }
...
void DWA::planning()
{
    ...
    cout << "[dwa]: raw dwa speed:(" << speed[0] << ", " << speed[1] << ")" << endl;

    double w_ratio = abs(speed[1] / max_angular_speed);
    double w_penalty = w_ratio * 0.8;
    if (speed[0] * w_penalty > 0.2) {
        speed[0] *= w_penalty;
    }
    cout << "[dwa]: final dwa speed:(" << speed[0] << ", " << speed[1] << ")" << endl;
...
int main(int argc, char** argv){
    ...
    n.param<double>("dwa/forecast_time", forecast_time, 1.0);
